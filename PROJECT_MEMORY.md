...
- **Host MVP**: Deployed the full behavioral verification platform and clinical triage queue to the production environment (`https://triageguard.ca`) over secure HTTPS, successfully integrating a local host-bound `tinydolphin` Ollama agent for CPU-based inference.
...
- **Remote Production Deployment**: Built AMD64 Docker images locally with the separate production API key baked into Nginx, loaded them on the VPS at `triageguard.ca`, and restarted the containers. Verifications show the system is fully healthy.
